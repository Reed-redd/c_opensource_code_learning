...
static ListNode *list_free(ListNode *const head, const CleanupListValue cleanup_value,
			   void *const cleanup_value_data);

static void add_symbol_value(ListNode *const symbol_map_head,
			     const char *const symbol_names[],
			     const size_t number_of_symbol_names, const void *value,
			     const int count);
...
static const int exception_signals[] = {
	SIGFPE, SIGILL, SIGSEGV, SIGBUS, SIGSYS,
};
...
static void set_source_location(SourceLocation *const location, const char *const file,
				const int line)
{
	assert_true(location);
	location->file = file;
	...
	if (list_find(head, symbol_name, symbol_names_match, &target_node)) {
		...
		if (number_of_symbol_names == 1) {
			...
			*output	     = (void *) value_node->value;
			...
			return_value =
				get_symbol_value(child_list, &symbol_names[1],
	...
	for (current = map_head->next; current != map_head; current = current->next) {
		...
		if (!list_empty(child_list)) {
			if (number_of_symbol_names == 1) {
				...
				for (child_node = child_list->next;
...
void *_mock(const char *const function, const char *const file, const int line)
{
	...
	if (rc) {
		...
		if (source_location_is_set(&global_last_mock_value_location)) {
			print_error("Previously returned mock value was declared "
				    "at " SOURCE_LOCATION_FORMAT "\n",
...
void _will_return(const char *const function_name, const char *const file, const int line,
		  const void *const value, const int count)
{
...
static int values_equal_display_error(const void *const left, const void *const right)
{
 ...
 * an error is displayed and 0 is returned. */
static int values_not_equal_display_error(const void *const left, const void *const right)
{
 ...
 * displayed. */
static int value_in_set_display_error(const void *value,
...
static int integer_in_range_display_error(const int value, const int range_min,
					  const int range_max)
{
...
static int integer_not_in_range_display_error(const int value, const int range_min,
					      const int range_max)
{
	if (value < range_min || value > range_max) {
		return 1;
...
static int memory_equal_display_error(const char *a, const char *b, const size_t size)
{
	int differences = 0;
	size_t i;
	for (i = 0; i < size; i++) {
 ...
 * returned. */
static int memory_not_equal_display_error(const char *a, const char *b, const size_t size)
{
	int same = 0;
	size_t i;
	for (i = 0; i < size; i++) {
...
{
	return value_in_set_display_error(value, (CheckIntegerSet *) check_value_data, 0);
}
...
{
	return value_in_set_display_error(value, (CheckIntegerSet *) check_value_data, 1);
}
...
{
	CheckIntegerSet *const check_integer_set = malloc(
	...
	_expect_check(function, parameter, file, line, check_function, check_integer_set,
		      &check_integer_set->event, count);
}
...
static int check_in_range(const void *value, void *check_value_data)
{
	...
	return integer_in_range_display_error((int) value, check_integer_range->minimum,
...
static int check_not_in_range(const void *value, void *check_value_data)
{
	...
	return integer_not_in_range_display_error(
		(int) value, check_integer_range->minimum, check_integer_range->maximum);
}
...
static void expect_range(const char *const function, const char *const parameter,
			 const char *const file, const int line, const int minimum,
			 const int maximum, const CheckParameterValue check_function,
			 const int count)
{
...
void _expect_in_range(const char *const function, const char *const parameter,
		      const char *const file, const int line, const int minimum,
		      const int maximum, const int count)
{
	expect_range(function, parameter, file, line, minimum, maximum, check_in_range,
		     count);
}
...
void _expect_value(const char *const function, const char *const parameter,
		   ...
		   const int count)
{
	_expect_check(function, parameter, file, line, check_value, (void *) value, NULL,
		      count);
}
...
void _expect_not_value(const char *const function, const char *const parameter,
		       ...
		       const int count)
{
	_expect_check(function, parameter, file, line, check_not_value, (void *) value,
		      NULL, count);
}
...
void _expect_not_string(const char *const function, const char *const parameter,
			...
			const int count)
{
	_expect_check(function, parameter, file, line, check_not_string, (void *) string,
		      NULL, count);
}
				...
				const CheckParameterValue check_function, const int count)
{
	...
	_expect_check(function, parameter, file, line, check_function, check_data,
		      &check_data->event, count);
}
...
{
	expect_memory_setup(function, parameter, file, line, memory, size, check_memory,
			    count);
}
...
static int check_any(const void *value, void *check_value_data) { return 1; }
...
{
	_expect_check(function, parameter, file, line, check_any, NULL, NULL, count);
}
		     ...
		     const char *file, const int line, const void *value)
{
	...
	const int rc = get_symbol_value(&global_function_parameter_map_head, symbols, 2,
					&result);
	if (rc) {
		...
		if (source_location_is_set(&global_last_parameter_location)) {
			print_error("Previously declared parameter value was declared "
				    "at " SOURCE_LOCATION_FORMAT "\n",
				    global_last_parameter_location.file,
				    global_last_parameter_location.line);
		} else {
...
void _assert_true(const int result, const char *const expression, const char *const file,
		  const int line)
{
	if (!result) {
		print_error("%s\n", expression);
...
void _assert_string_not_equal(const char *const a, const char *const b, const char *file,
			      const int line)
{
...
void _assert_memory_equal(const void *const a, const void *const b, const size_t size,
			  const char *const file, const int line)
{
...
void _assert_memory_not_equal(const void *const a, const void *const b, const size_t size,
			      const char *const file, const int line)
{
...
void _assert_not_in_range(const int value, const int minimum, const int maximum,
			  const char *const file, const int line)
{
...
void *_test_calloc(const size_t number_of_elements, const size_t size, const char *file,
		   const int line)
{
	...
	// Check the guard blocks.
	{
		...
		for (i = 0; i < ARRAY_LENGTH(guards); i++) {
			...
			for (j = 0; j < MALLOC_GUARD_SIZE; j++) {
				...
				if (diff) {
					print_error("Guard block of 0x%08x size=%d "
						    "allocated by " SOURCE_LOCATION_FORMAT
						    " at 0x%08x is corrupt\n",
		...
		if (code == code_info->code) {
			...
			if (!shown_debug_message) {
				print_error(
					...
					"2. Change 'Files of type' to 'Executable "
					"Files'\n"
					"3. Open this executable.\n"
					"4. Select menu item Debug->Start\n"
					"\n"
					"Alternatively, set the environment variable \n"
					"UNIT_TESTING_DEBUG to 1 and rebuild this "
					"executable, \n"
...
{
	const ListNode *const check_point =
